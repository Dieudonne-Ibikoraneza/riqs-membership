...
export default function AdminOverview() {
  ...
  const stats = [
    { i: ClipboardList, label: "Pending", v: pending, c: "text-amber-600", bg: "bg-amber-50 dark:bg-amber-950/20" },
    { i: Users, label: "Under Review", v: review, c: "text-blue-600", bg: "bg-blue-50 dark:bg-blue-950/20" },
    { i: AlertTriangle, label: "Correction Required", v: correction, c: "text-orange-600", bg: "bg-orange-50 dark:bg-orange-950/20" },
    { i: CheckCircle2, label: "Approved", v: approved, c: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-950/20" },
  ];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground font-sans">Snapshot of current applications and member activity.</p>
      </div>
          ...
          >
            <Card className="hover-lift border-zinc-100 dark:border-zinc-800">
              <CardContent className="p-5">
                <div className={`mb-2 inline-flex h-9 w-9 items-center justify-center rounded-md ${s.bg} ${s.c}`}>
        ...
        <CardContent className="space-y-2">
          {APPLICATIONS.slice(0, 6).map((a, index) => (
            ...
            >
              ...
              >
                <div>
                  <div className="font-semibold text-zinc-800 dark:text-zinc-200">{a.applicantName}</div>
                  <div className="text-xs text-muted-foreground font-mono">
                    {a.id} · {a.category} · {a.practiceLocation}
                  </div>
                </div>
                <Badge variant="outline" className="border-zinc-200 dark:border-zinc-700 bg-white/50 dark:bg-black/50">
                  {a.status}
                </Badge>
              </Link>
            </motion.div>
