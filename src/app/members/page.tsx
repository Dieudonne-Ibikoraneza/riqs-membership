...
export default function MembersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    arr.sort((a, b) => {
      const k = sort;
      const av =
        k === "name" ? a.fullName : k === "id" ? a.membershipId : a.category;
      const bv =
        k === "name" ? b.fullName : k === "id" ? b.membershipId : b.category;
      ...
      <main className="flex-1">
        ...
        <section className="relative overflow-hidden brand-gradient text-white">
          ...
          <div className="relative mx-auto max-w-7xl px-4 py-16 animate-fade-in z-10">
            ...
            >
              <BadgeCheck className="mr-1 h-3 w-3 text-gold fill-gold" />{" "}
              Verified register
            </Badge>
            <h1 className="mt-4 text-4xl font-bold md:text-5xl leading-tight">
              Public Members Directory
            </h1>
            <p className="mt-3 max-w-2xl text-white/80 leading-relaxed font-sans">
              Search, filter and verify all{" "}
              <span className="font-semibold gold-text">approved</span> RIQS
              members. Every record is publicly verifiable.
            </p>
            <div className="mt-6 flex items-center gap-6 text-sm">
              <div className="flex items-center gap-2">
                <Users className="h-4 w-4 gold-text" />
                <span>
                  <span className="font-bold">{APPROVED.length}</span> active
                  members
                </span>
              </div>
            </div>
                ...
                <div className="flex flex-wrap items-center gap-2 text-sm">
                  ...
                  {activeFilters > 0 && (
                    <>
                      ...
                      <button
                        onClick={reset}
                        className="text-xs text-navy dark:text-gold underline-offset-4 hover:underline"
                      >
                        Clear all
                      </button>
                    </>
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-none font-semibold">
                            <BadgeCheck className="mr-1 h-3.5 w-3.5 text-emerald-600 inline shrink-0" />{" "}
                            Approved
                          </Badge>
                        </td>
                      </tr>
            ...
            <CardContent className="relative p-8 flex flex-wrap items-center justify-between gap-4 z-10">
              <div>
                ...
                <p className="mt-1 text-sm text-white/80 font-sans">
                  Join the official register of Quantity Surveyors in Rwanda.
                </p>
              </div>
              <Link href="/register">
                <Button
                  size="lg"
                  className="bg-gold text-[#1a1a1a] hover:bg-gold/90 shadow-gold h-12 px-6 border-none font-bold"
                >
                  Apply now
                </Button>
              </Link>
  ...
  return (
    <Card className="group hover-lift overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 shadow-sm">
      ...
      <CardContent className="pt-10 pb-5 px-5">
        <div className="text-[10px] uppercase tracking-wider gold-text font-bold">
          {m.membershipId}
        </div>
        <div className="mt-1 text-base font-bold text-navy dark:text-white leading-tight">
          {m.fullName}
        </div>
        <div className="mt-1.5">
          <Badge
            variant="outline"
            className="border-navy/15 bg-navy/5 text-navy dark:border-zinc-800 dark:text-zinc-350 text-[10px] font-semibold"
          >
            {m.category}
          </Badge>
        </div>
        <div className="mt-4 space-y-2 text-xs text-muted-foreground font-sans">
          ...
          <div className="flex items-center gap-2">
            <Phone className="h-3.5 w-3.5 text-gold shrink-0" /> {m.phone}
          </div>
