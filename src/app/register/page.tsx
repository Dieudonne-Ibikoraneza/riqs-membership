  ...
  return (
    <div className="grid min-h-screen md:grid-cols-2">
            ...
            {!pending ? (
              <>
                ...
                <div className="mt-6 text-center text-sm text-muted-foreground">
                  Already have an account? <Link href="/login" className="font-semibold text-navy hover:underline">Sign in</Link>
                </div>
              </>
            ) : (
              <div className="animate-fade-in">
                <h1 className="text-2xl font-bold text-navy">Verify your email</h1>
                <p className="mt-1 text-sm text-muted-foreground">Enter the 4-digit code we sent to <strong>{pending.email}</strong>.</p>
                {devCode && (
                ...
                <Button onClick={verify} className="mt-6 w-full h-11 bg-gold text-[#1a1a1a] hover:bg-gold/90 shadow-gold font-semibold">Verify & create account</Button>
                <Button variant="ghost" onClick={cancelPending} className="mt-2 w-full">Back</Button>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <div className="hidden brand-gradient md:flex md:flex-col md:justify-between md:p-12 text-white order-1 md:order-2">
        ...
        <div>
          <h2 className="text-3xl font-bold">Join Rwanda's QS register.</h2>
        ...
        <div className="text-xs text-white/60">© {new Date().getFullYear()} RIQS</div>
      </div>
    </div>
  );
}
