...
export default function Application() {
  ...
  const [data, setData] = useState<any>({
    ...
    education: [{ degree: "Bachelor’s Degree", institution: "", year: 2026, studyField: "", startMonthYear: "" }],
  ...
  const STEPS = useMemo(() => {
    const list = ["Practice Location", "Entity Type", "Category", "Profile Details", "Employment Record"];
  ...
  const updateLocation = (loc: string) => {
    const nextCat = data.entityType === "Individual"
      ? (loc === "Local" ? "Graduate" : "Technologist")
      : (loc === "Local" ? "Small Firm - Annual turnover less than 50 Million RWF" : "Small Firm - Annual turnover less than 100,000 USD");
    setData((d: any) => ({ ...d, practiceLocation: loc, category: nextCat }));
  };

  const updateEntity = (ent: string) => {
    const nextCat = ent === "Individual"
      ? (data.practiceLocation === "Local" ? "Graduate" : "Technologist")
      : (data.practiceLocation === "Local" ? "Small Firm - Annual turnover less than 50 Million RWF" : "Small Firm - Annual turnover less than 100,000 USD");
  ...
  const documentChecklist = useMemo(() => {
    ...
    if (data.entityType === "Individual") {
      ...
      const fee = data.category.includes("Small") ? (isLocal ? "100,000 RWF" : "100 USD") : data.category.includes("Medium") ? (isLocal ? "200,000 RWF" : "200 USD") : (isLocal ? "300,000 RWF" : "300 USD");
                ...
                data.entityType === "Individual" ? (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="border border-zinc-100 dark:border-zinc-800 p-4 rounded-md bg-zinc-50/50 dark:bg-zinc-950/20 space-y-4">
                      <h3 className="font-semibold text-base text-navy">Personal / Representative Information</h3>
              ...
              {currentStepName === "Education History" && (
                <div className="space-y-4">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setData({ ...data, education: [...data.education, { degree: "Bachelor’s Degree", institution: "", year: 2026, studyField: "", startMonthYear: "" }] })}
                    className="border-zinc-200 dark:border-zinc-800"
                  >
                    + Add Academic Record
                  </Button>
              ...
              {currentStepName === "Mentorship Plan" && (
                <div className="space-y-4">
                  <div className="border border-dashed border-zinc-250 dark:border-zinc-800 p-4 text-xs text-muted-foreground rounded-md leading-relaxed font-sans">
                    As a Graduate applicant, local mentorship is highly recommended for promotion to Technologist or Professional standing. You can register up to <strong className="text-navy">5 Mentors</strong> maximum.
                  </div>
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-1">
                      <Label htmlFor="ment-name">Mentor's Full Name</Label>
                      <Input id="ment-name" placeholder="Leave blank if none" value={data.mentor.name} onChange={e => setData({ ...data, mentor: { ...data.mentor, name: e.target.value } })} />
                    </div>
                    <div className="space-y-1">
                      <Label htmlFor="ment-id">Mentor's RIQS Registration ID</Label>
                      <Input id="ment-id" placeholder="e.g. RIQS-2023-P-022" value={data.mentor.regId} onChange={e => setData({ ...data, mentor: { ...data.mentor, regId: e.target.value } })} />
                    </div>
                    <div className="md:col-span-2 space-y-1">
                      <Label htmlFor="ment-plan">Mentorship Goal / Plan Summary</Label>
                      <Textarea id="ment-plan" rows={4} placeholder="Briefly summarize your practical experience milestones..." value={data.mentor.plan} onChange={e => setData({ ...data, mentor: { ...data.mentor, plan: e.target.value } })} />
                    </div>
                  </div>
                </div>
              )}
                  ...
                  <div className="bg-zinc-50 dark:bg-zinc-950/40 p-5 text-sm space-y-2.5 rounded-md border border-zinc-100 dark:border-zinc-800">
                    {[
                      ...
                      ...(data.entityType === "Individual" ? [
                        ...
                        ["Employment Record Rows", `${data.employment.filter((x: any) => x.company).length} items`],
                      ] : [
                      ...
                      <div key={k as string} className="grid grid-cols-2 gap-2 pb-2 border-b border-zinc-50 dark:border-zinc-800/40 last:border-0 last:pb-0 font-sans">
